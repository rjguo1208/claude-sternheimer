...
def diagT(Xsub,tag):
    ...
    for j,om in enumerate(omall):
        Ts=Tsub_of(om/RY,Xsub).reshape(ns,nb,ns,nb)
    ...
    return out
ImM=diagT(Msub,"M  (bare)"); ImV=diagT(Vsub,"V~ (full)")
...
def rates(Im):                                       # hbar/tau in meV at n_d=ND
    ...
    return r
rM=rates(ImM); rV=rates(ImV)
eps=np.array([s[2] for s in states]); side=np.array([s[3] for s in states])
np.savez("p7_scatrate.npz",eps=eps,side=side,rM=rM,rV=rV,VBM=VBMe,CBM=CBMe,nd=ND,eta=ETA*RY,
...
for ax,sd,edge,lbl in [(axs[0],"V",VBMe,"VBM"),(axs[1],"C",CBMe,"CBM")]:
    ...
    ax.scatter(x,rM[m],s=9,c="#2a7a2a",alpha=.55,label="11 bands, bare $M$")
    ax.scatter(x,rV[m],s=9,c="#6a4c93",alpha=.55,label=r"11 bands + full-order $\Sigma_{\rm full}$ ($\tilde V$, fesh60)")
...
axs[0].invert_xaxis()
h,l=axs[0].get_legend_handles_labels(); fig.legend(h,l,ncol=2,loc="upper center",fontsize=10,frameon=False)
fig.suptitle(r"Defect-limited scattering rate  $\hbar/\tau_{n\mathbf{k}}=-2 n_d\,\mathrm{Im}\,T_{nn}(\mathbf{k},\varepsilon_{n\mathbf{k}}+i\eta)$"
             rf"   ($\eta$={ETA*RY*1e3:.0f} meV, $n_d$=1%)",y=1.06)
plt.tight_layout(); plt.savefig("p7_scatrate.png",dpi=140,bbox_inches="tight")
...
for sd,edge,lbl in [("V",VBMe,"VBM"),("C",CBMe,"CBM")]:
    ...
    for d0,d1 in [(0.0,0.05),(0.05,0.15),(0.15,0.30)]:
        ...
        if m.sum()==0: continue
        a,b=rM[m].mean(),rV[m].mean()
        print(f"  |e-edge| in [{d0:.2f},{d1:.2f}) eV  ({m.sum():4d} states): "
              f"bare M {a:8.3f} meV (tau {HBAR/a:8.1f} fs) | full V~ {b:8.3f} meV "
              f"(tau {HBAR/b:8.1f} fs) | ratio V~/M {b/a:6.3f}")
