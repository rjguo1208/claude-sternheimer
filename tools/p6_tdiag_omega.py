...
ax[1].plot(omv,np.abs(Tpp),c="#1f77b4",lw=2.0,label=r"$|T_{PP}|$ with rest-space")
ax[1].plot(omv,np.abs(Tm),c="#ff7f0e",lw=1.6,label=r"$|T_M|$ no rest-space")
ax[1].axvline(omega0*RY,ls="--",c="green",lw=1.5,label=r"$\omega_0=$VBM")
ax[1].axhline(abs(V[aK,aK]),ls=":",c="#1f77b4"); ax[1].axhline(abs(M[aK,aK]),ls=":",c="#ff7f0e")
for L in near: ax[1].plot([L*RY],[0],"v",c="purple",ms=7)
ax[1].set_xlim(omega0*RY-1.5, omega0*RY+0.6)
ax[1].set_xlabel(r"$\omega$  (eV)"); ax[1].set_ylabel(r"$|T(K,K;\omega)|$  (Ry)")
ax[1].set_title(r"Band-edge zoom (▼ = resonance poles $\lambda_n$)"); ax[1].legend(fontsize=8); ax[1].grid(alpha=.3)
