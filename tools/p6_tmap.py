...
Self-energy Sigma_e-def(nk,omega) = n_d * T_{nk,nk}(omega); here the FULL T (with rest-space, V~).
Key: the Koster-Slater T_sub(omega)=[1-V~^W G^A(omega)]^{-1}V~^W is k-INDEPENDENT (only V~ and the host
G^A(omega) enter) -> solve once per omega, then T(nk,omega)=l(k).T_sub(omega).r(k) for all path-k via
the Wannier interpolation + top-band projection. Re and Im maps over omega in [-3.5,-0.5] eV."""
...
def Tsub_of(omega):
    ...
    return np.linalg.solve(np.eye(dim)-Vsub@gt,Vsub)

# ---- path Gamma-M-K: interpolation+top-band-projection vectors l(k), r(k) (omega-independent) ----
...
for i,k in enumerate(path):
    ...
    lvec[i]=(phb[:,None]*vt.conj()[None,:]).reshape(dim)      # bra: -k.R', top band
    rvec[i]=(phk[:,None]*vt[None,:]).reshape(dim)             # ket: +k.R', top band
...
# ---- sweep omega: one solve per omega, evaluate all k ----
omega=np.linspace(WLO,WHI,NW)/RY
Tmap=np.empty((Nk,NW),complex)
for j,om in enumerate(omega):
    Ts=Tsub_of(om)
    Tmap[:,j]=((lvec@Ts)*rvec).sum(1)
    if j%30==0: print(f"  {j}/{NW}",flush=True)
iK=Nk-1; jVBM=int(np.argmin(np.abs(omega-omega0)))
print(f"check T(K, omega=VBM)={Tmap[iK,jVBM]:+.4f}  (Figure-3 on-shell was -0.0328-0.0969i)")
print(f"Re range [{Tmap.real.min():.3f},{Tmap.real.max():.3f}]  Im range [{Tmap.imag.min():.3f},{Tmap.imag.max():.3f}]")

fig,ax=plt.subplots(1,2,figsize=(14,5.4))
# Re: diverging, symmetric about 0 (level shift)
...
# -Im: sequential, >=0 (spectral weight / scattering rate)
...
plt.suptitle(r"Diagonal $T(nk,\omega)$ (top valence band, full $T$ with rest-space) along Γ–M–K — dashed = on-shell $\varepsilon_{\rm top}(k)$",y=1.02)
plt.tight_layout(); plt.savefig("p6_tmap.png",dpi=130,bbox_inches="tight"); print("wrote p6_tmap.png")
