...
CATALOG = [
    ...
    ("Substitutional defects O$_S$ / Se$_S$ (explicit-60 + spectral)", "Result", "2026-06-15", "warn", "Caveat",
     ...
     "<strong>CAVEAT:</strong> direct DFT of O$_S$ shows NO in-gap state &mdash; the substitution in-gap "
     "features (Figs 25&ndash;29) are a likely artifact of the new-species nonlocal e-d matrix element "
     "(zero-atom $d_{\\mathrm{van}}$), under diagnosis. The diagonalization route reproduces the vacancy "
     "$a_1{\\oplus}e$ exactly (Fig 29), so only the vacancy results are DFT-validated.",
