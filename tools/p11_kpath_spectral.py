...
import sys
CACHE = sys.argv[1] if len(sys.argv) > 1 else "/home/rjguo/edi_tmatrix/sternA/vsrlx/kpath_T450.npz"
ZOOM = (float(sys.argv[2]), float(sys.argv[3])) if len(sys.argv) > 3 else None
OUTTAG = sys.argv[4] if len(sys.argv) > 4 else ""
z = np.load(CACHE)
...
for p, (nd, lab) in enumerate(NDS):
    A = spectral(nd)
    if vmax is None:
        m = (E >= ZOOM[0]) & (E <= ZOOM[1]) if ZOOM else np.ones(len(E), bool)
        vmax = np.percentile(A[m], 99.5)
    ...
    ax[p].set_xlim(0, nkp-1); ax[p].set_ylim(*(ZOOM if ZOOM else (E.min(), E.max())))
...
out = "/home/rjguo/edi_tmatrix/sternA/claude-sternheimer/docs/assets/kpath_spectral%s.png" % OUTTAG
