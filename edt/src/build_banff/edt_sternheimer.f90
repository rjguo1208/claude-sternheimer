...
CONTAINS
  ...
  SUBROUTINE solve_rest_cg(kp, s, omega0_ry, alpha_ry, nact, evc_act, thr, maxiter, &
    ...
    COMPLEX(dp), ALLOCATABLE :: r(:), p(:), Ap(:), z(:), hp(:,:), ptmp(:,:), ovlv(:)
    ...
    ALLOCATE(ovlv(MAX(nact,1)))
    DO ig = 1, npw
       prec(ig) = 1.0_dp / MAX(g2kin(ig), 1.0_dp)
    ENDDO
    ! becp for h_psi is allocated by the CALLER (once per channel, not per solve)
    ...
    DO it = 1, maxiter
       ...
       IF (nact > 0) THEN
          ! alpha * P_act p  via two ZGEMVs (was a per-state SUM/axpy loop)
          CALL ZGEMV('C', npw, nact, (1.0_dp,0.0_dp), evc_act, SIZE(evc_act,1), p, 1, &
                     (0.0_dp,0.0_dp), ovlv, 1)
          CALL ZGEMV('N', npw, nact, CMPLX(alpha_ry,0.0_dp,dp), evc_act, SIZE(evc_act,1), &
                     ovlv, 1, (1.0_dp,0.0_dp), Ap, 1)
       ENDIF
    ...
    DEALLOCATE(r, p, Ap, z, hp, ptmp, prec, ovlv)
  ...
  SUBROUTINE vtilde_block_mpi(xkc, xkcr, omega0_ry, win_min_ry, win_max_ry, &
                              nbndskip_in, thr, nk_block_in, iket_band, iket_ki, outfile, born_flag)
    !-------------------------------------------------------------------------
    ...
    IF (ionode) THEN
       ...
       IF (born_flag) WRITE(stdout,'(5X,A)') &
            'BORN-ONLY: rest Sternheimer solves skipped (Vtilde = Born M).'
       FLUSH(stdout)
    ENDIF
    ...
    ! bec for ALL states of a k-group in one ZGEMM (the former per-state SUM loops
    ! were the dominant serial cost: ~0.3 s/state x N_A, unvectorized).
    ALLOCATE(coeff_d_all(nkb_d,N_A), coeff_p_all(nkb_p,N_A))
    ALLOCATE(vkb_d(npwx,nkb_d), vkb_p(npwx,nkb_p))
    ALLOCATE(bec_d(nkb_d,na_max), bec_p(nkb_p,na_max))
    kg_prev = -1
    DO a = 1, N_A
       ...
       IF (kg /= kg_prev) THEN
          ...
          CALL ZGEMM('C','N', nkb_d, na_k(kg), ngk_all(kg), cone, vkb_d, npwx, &
                     evc_act_all(1,1,kg), n2, czero, bec_d, nkb_d)
          CALL ZGEMM('C','N', nkb_p, na_k(kg), ngk_all(kg), cone, vkb_p, npwx, &
                     evc_act_all(1,1,kg), n2, czero, bec_p, nkb_p)
          kg_prev = kg
       ENDIF
       CALL make_coeff(V_d%nat,V_d%ityp,V_d%ntyp,nkb_d,bec_d(:,slot),coeff_d_all(:,a))
       CALL make_coeff(V_p%nat,V_p%ityp,V_p%ntyp,nkb_p,bec_p(:,slot),coeff_p_all(:,a))
    ENDDO
    ...
    DO ik = 1, nks
       ...
       DO a = 1, N_A
          ...
          ENDDO
       ENDDO
       ! KB (nonlocal) part: one rank-nkb update for ALL kets at once (BLAS3),
       ! replacing the former per-ket rank-1 loops.  Single-ket mode keeps the
       ! scalar path so untouched columns stay exactly zero.
       IF (do_single) THEN
          DO ikb = 1, nkb_d
             S(1:npw_kp,a_sel) = S(1:npw_kp,a_sel) + vkb_d(1:npw_kp,ikb)*coeff_d_all(ikb,a_sel)
          ENDDO
          DO ikb = 1, nkb_p
             S(1:npw_kp,a_sel) = S(1:npw_kp,a_sel) - vkb_p(1:npw_kp,ikb)*coeff_p_all(ikb,a_sel)
          ENDDO
       ELSE
          CALL ZGEMM('N','N', npw_kp, N_A, nkb_d,  cone, vkb_d, npwx, &
                     coeff_d_all, nkb_d, cone, S, npwx)
          CALL ZGEMM('N','N', npw_kp, N_A, nkb_p, -cone, vkb_p, npwx, &
                     coeff_p_all, nkb_p, cone, S, npwx)
       ENDIF
       ...
       ! (born_flag skips the whole block: Vtilde = Born M only, Sgblk stays zero)
       IF (.NOT. born_flag) THEN
          DO a = 1, N_A
             IF (do_single .AND. a /= a_sel) CYCLE
             CALL apply_Qproj(npw_kp, npol, nact_kp, evc_act_all(:,:,kpg), S(:,a))
          ENDDO
          CALL allocate_bec_type(nkb, 1, becp)     ! once per channel, not per solve
          DO a = 1, N_A
             IF (do_single .AND. a /= a_sel) CYCLE
