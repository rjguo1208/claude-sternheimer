...
CONTAINS
  ...
  SUBROUTINE build_V_folded(q_cryst, Vf)
    ...
    LOGICAL  :: isq0
    INTEGER, ALLOCATABLE, SAVE :: fold_map(:)

    ! -- optimized: q-independent supercell->primitive index map built once
       ...
       DO irz = 0, V_d%nr3 - 1
          ...
          DO iry = 0, V_d%nr2 - 1
             ...
             DO irx = 0, V_d%nr1 - 1
                ...
                inr = inr + 1
                fold_map(inr) = ir3mod * dffts%nr1 * dffts%nr2 + ir2mod * dffts%nr1 + ir1mod + 1
             ENDDO
          ENDDO
       ENDDO
    ENDIF
    ...
    IF (isq0) THEN
       DO inr = 1, V_d%nr1 * V_d%nr2 * V_d%nr3
          irnmod = fold_map(inr)
    ...
    ELSE
       ...
       BLOCK
         COMPLEX(dp) :: px(0:V_d%nr1-1), py(0:V_d%nr2-1), pz(0:V_d%nr3-1), pyz
         DO irx = 0, V_d%nr1 - 1
            arg = irx * d1; px(irx) = CMPLX(COS(arg), SIN(arg), dp)
         ENDDO
         DO iry = 0, V_d%nr2 - 1
            arg = iry * d2; py(iry) = CMPLX(COS(arg), SIN(arg), dp)
         ENDDO
         DO irz = 0, V_d%nr3 - 1
            arg = irz * d3; pz(irz) = CMPLX(COS(arg), SIN(arg), dp)
         ENDDO
         inr = 0
         DO irz = 0, V_d%nr3 - 1
            DO iry = 0, V_d%nr2 - 1
               ...
               DO irx = 0, V_d%nr1 - 1
                  inr = inr + 1
                  irnmod = fold_map(inr)
                  Vf(irnmod) = Vf(irnmod) + V_colin(inr) * (px(irx) * pyz)
               ENDDO
            ENDDO
         ENDDO
       END BLOCK
