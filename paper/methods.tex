...
\subsection{Krylov construction}

Equation~\eqref{eq:SigmaR} is a resolvent of the dressed rest operator
$P_R H P_R$ between the $N_A$ source vectors
\begin{equation}
  |\chi_a\rangle = P_R\, \dV\, |\psi_a\rangle .
\end{equation}
We block-tridiagonalize $P_R H P_R$ in the Krylov space generated by the block
$\chi$, with block width $N_A$,
\begin{equation}
  P_R H P_R\, Q_j = Q_{j-1} B_{j-1}^{\dagger} + Q_j A_j + Q_{j+1} B_j,
  \qquad Q_0 R_0 = \mathrm{qr}(\chi).
  \label{eq:lanczos}
\end{equation}
Full reorthogonalization against all stored blocks is used; the three-term
recurrence alone loses orthogonality catastrophically and produces ghost
eigenvalues. This is why all blocks $Q_j$ must be retained, and it is the
memory bottleneck of the method (Sec.~\ref{sec:cost}).

Each step requires two operator applications with very different costs.
Applying $H_0$ is diagonal in $\kk$: kinetic energy in $\GG$-space, one pair of
FFTs for the local potential, and a projector \textsc{zgemm} for the nonlocal
part. Applying $\dV$ scatters, so every output channel couples to every source
channel,
\begin{equation}
  (\dV \chi)_\kk(\rr) = \sum_{\kk'} V_{\kk-\kk'}(\rr)\, \chi_{\kk'}(\rr),
  \label{eq:applydV}
\end{equation}
a triple loop over grid points, output channels and source channels. It
dominates the step and sets the scaling of Sec.~\ref{sec:cost}.
...
application around the ninth block and is the wall beyond this size; selective
reorthogonalization, or the pole--residue representation of
Sec.~\ref{sec:lanczos}, are the levers.
