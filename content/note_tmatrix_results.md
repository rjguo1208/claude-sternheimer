...
To separate what the *chemical* substitution does from what the *lattice distortion* does, we recompute
both defects with the **unrelaxed** geometry (O/Se frozen at the ideal S site) and **no potential
alignment** (`pot_align='none'` instead of `vacuum`). The local $\Delta V$ changes a lot: its peak for
O$_S$ shrinks $\sim4\times$ ($V_{\rm colin}\in[-0.04,+2.5]$ Ry unrelaxed vs $[-6.4,+9.9]$ relaxed — the
ionic displacement dominates O$_S$'s perturbation), while Se$_S$ stays strong ($[-12.3,+0.3]$ Ry — the
deep Se local pseudopotential from its size + $3s3p3d$ semicore). The alignment is a $\sim47$ meV
constant, minor by comparison.

| $A_{\rm peak}$ (in-gap, $\tilde V$, $n_d{=}1/36$) and levels (rel. VBM) | relaxed + vacuum-aligned | unrelaxed + no-align |
|---|---|---|
| **O$_S$** | $A_{\rm peak}=27.9$/eV; shallow $+0.04/{+}0.04/{+}0.10$ + doublet $+0.73$ | $A_{\rm peak}=251.5$/eV; doublet $+0.725$ + singlet $+1.55$ |
| **Se$_S$** | $A_{\rm peak}=12.7$/eV; **dense $\sim15$ levels** $+0.02\ldots+0.45$ + $+1.19$ | $A_{\rm peak}=159.6$/eV; **single doublet $+0.48$** |

![O_S spectral function, unrelaxed geometry and no alignment: sharp dispersive host bands with an isolated in-gap doublet, in contrast to the relaxed case.](../assets/sub_spectral_OS_unrelax.png)

*Figure 27. O$_S$, **unrelaxed + no-align**. The host bands are **much sharper** than the relaxed case
(Fig 25); the in-gap peak is $9\times$ taller.*

![Se_S spectral function, unrelaxed geometry and no alignment: the heavy valence-edge broadening of the relaxed case is gone, leaving sharp bands and one in-gap doublet.](../assets/sub_spectral_SeS_unrelax.png)

*Figure 28. Se$_S$, **unrelaxed + no-align**. The heavy valence-edge broadening of Fig 26 **disappears** —
the bands are sharp and only one in-gap doublet remains.*

The lesson: the **band broadening and the dense near-VBM level manifolds of the relaxed substitutions
are generated by the ionic relaxation** (the lattice distortion perturbing many valence states), not by
the bare chemical substitution. Frozen at the ideal lattice, both defects give **sharp bands and
isolated in-gap levels** (peaks $9$–$13\times$ taller). The **deep doublet is the robust intrinsic
