...
**实现状态。** $r_3$ 已用 explicit $M$ 路精确测出(上小节,$0.37$–$0.45$);Fortran 里的逐阶 ladder 仍待落地。最小可行实现(MVP):

1. 复用现有二阶 solve 拿到 $\chi^0$(已有);
2. 加一个 $\Delta V$ 作用例程(局域超胞 FFT + 非局域可分离),在 code 内白送地算 $\Sigma^{(3)}$,交叉验证 explicit 路的 $r_3$;
