...
即把 $\Delta V_{QQ}$ 放进 CG 的算符里、一次解到全阶。逐阶阶梯的价值在于:它把这个非微扰解**展开成可监控的序列**,既能在 $\rho<1$ 时按需截断,又能用 $\Sigma^{(3)},\Sigma^{(5)},\ldots$ 的大小直接读出 $\rho$、判断该不该信二阶。
...
S-vacancy 的二阶把 $e$ 过度束缚 $\sim1.1$ eV,提示 $\rho\sim\mathcal O(1)$,大概率落在后两栏。
...
**实现状态。** 本页是推导与方案;高阶阶梯尚未落到 EDT 代码里。最小可行实现(MVP):

1. 复用现有二阶 solve 拿到 $\chi^0$(已有);
2. 加一个 $\Delta V$ 作用例程(局域超胞 FFT + 非局域可分离),先**白送**地算 $\Sigma^{(3)}$、量出 $r_3$;
3. 若 $r_3\lesssim1$,$+1$ solve 上到五阶;若 $r_3\gtrsim1$,转全阶 Feshbach (3.4)。

无论哪条路,验收标准是把 dressed 的 $e$ 从二阶的 $+0.36$ eV 抬回到 explicit/DFT 的 $\sim+1.2\text{–}1.35$ eV。
