...
每个 $E$:host $k$-和 $\mathcal O(N_k\,n_{\rm band}\,\dim D^2)$ $+$ 行列式 $\mathcal O(\dim D^3)$,扫几十个 $E$ 或 root-find,总计**秒到分钟**,还能用更细的 $k$ 网格收敛。它**绕开**了 $P$/$Q$ 划分与 rest dressing(不展开 $\Delta V_{QQ}$,无 ladder 发散),又比 explicit 省掉整块 $M$ 与 $N_A^3$ 对角化。**地基已就位**:Wannier $M^W$ 的 gauge fix(Fig 14–15)、短程性 / $R_{\rm cut}=4$ 收敛(Fig 17–20)都在结果页验过。这是求 S-vacancy $a_1$+$e$ **最省又精确**的路线;实现与实测结果见 §7。
